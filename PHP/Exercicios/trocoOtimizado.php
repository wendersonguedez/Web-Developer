...
function troco($valor){
    $r = [0, 0, 0, 0, 0]; // array que ira retornar o resultado para o usuario a quantidade de notas necessarias para determinado troco
    $notas = [50, 20, 10, 5, 2]; // array de notas disponiveis para troco
    
    for($i=0; $i < count($r); $i++){
        $r[$i] = floor($valor / $notas[$i]); // esta linha fica responsavel para fazer o calculo de todas as notas pelo valor do troco. $r[$i] que dependendo do valor de $i, o resultado de determinada operação será armazenada na variavel $r
        $valor -= $r[$i] * $notas[$i]; // esta linha fica responsavel por atualizar o valor que ainda daremos de troco para cada iteração com a linha acima
    }
    return $r;
}
